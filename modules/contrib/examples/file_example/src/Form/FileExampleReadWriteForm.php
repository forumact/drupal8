...
class FileExampleReadWriteForm extends FormBase {
  ...
  public function handleUnmanagedPhp(array &$form, FormStateInterface $form_state) {
    ...
    for ($i = 0; $i < $length; $i += $write_size) {
      ...
      if ($result === FALSE) {
        $this->messenger()
          ->addMessage($this->t('Failed writing to the file %file', ['%file' => $destination]), 'error');
        fclose($fp);
        return;
      }
    }
  ...
  public function handleFileRead(array &$form, FormStateInterface $form_state) {
    ...
    if (empty($uri) or !is_file($uri)) {
      $this->messenger()
        ->addMessage($this->t('The file "%uri" does not exist', ['%uri' => $uri]), 'error');
      return;
    }
    ...
    if ($buffer) {
      ...
      $this->messenger()
        ->addMessage($this->t('Failed to retrieve the file %file', ['%file' => $uri]));
    }
  ...
  public function handleFileDelete(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($file_object)) {
      ...
      try {
        ...
        $this->messenger()
          ->addMessage($this->t('Successfully deleted managed file %uri', ['%uri' => $uri]));
    ...
    // Else use file_unmanaged_delete().
      ...
      if ($result !== TRUE) {
        $this->messenger()
          ->addMessage($this->t('Failed deleting unmanaged file %uri', [
            '%uri' => $uri,
            'error',
          ]));
      }
        ...
        $this->messenger()
          ->addMessage($this->t('Successfully deleted unmanaged file %uri', ['%uri' => $uri]));
  ...
  public function handleFileExists(array &$form, FormStateInterface $form_state) {
    ...
    if (is_file($uri)) {
      $this->messenger()
        ->addMessage($this->t('The file %uri exists.', ['%uri' => $uri]));
    }
    else {
      $this->messenger()
        ->addMessage($this->t('The file %uri does not exist.', ['%uri' => $uri]));
    }
  }
  ...
  public function handleDirectoryCreate(array &$form, FormStateInterface $form_state) {
    ...
    if (!file_prepare_directory($directory, FILE_MODIFY_PERMISSIONS | FILE_CREATE_DIRECTORY)) {
      $this->messenger()
        ->addMessage($this->t('Failed to create %directory.', ['%directory' => $directory]), 'error');
    }
    else {
      $result = is_dir($directory);
      $this->messenger()
        ->addMessage($this->t('Directory %directory is ready for use.', ['%directory' => $directory]));
  ...
  public function handleDirectoryDelete(array &$form, FormStateInterface $form_state) {
    ...
    if (!$result) {
      $this->messenger()
        ->addMessage($this->t('Failed to delete %directory.', ['%directory' => $directory]), 'error');
    }
    else {
      $this->messenger()
        ->addMessage($this->t('Recursively deleted directory %directory.', ['%directory' => $directory]));
      $this->setDefaultDirectory($directory);
    }
  }
  ...
  public function handleDirectoryExists(array &$form, FormStateInterface $form_state) {
    ...
    if (!$result) {
      $this->messenger()
        ->addMessage($this->t('Directory %directory does not exist.', ['%directory' => $directory]));
    }
    else {
      $this->messenger()
        ->addMessage($this->t('Directory %directory exists.', ['%directory' => $directory]));
    }
  }
  ...
  public function handleShowSession(array &$form, FormStateInterface $form_state) {
    ...
    if ($this->moduleHandler->moduleExists('devel')) {
      ...
      $this->messenger()
        ->addMessage('<pre>' . print_r($this->getStoredData(), TRUE) . '</pre>');
    }
  }
